...
void OEM_1S_GET_GPIO(ipmi_msg *msg)
{
	...
	for (uint8_t i = 0; i < gpio_cnt; i++) {
		...
		else if (i >= WORK_AROUND_BIOS_DEBUG_PIN_IDX_REAL && i < WORK_AROUND_BIOS_DEBUG_PIN_IDX)
			tmp_gpio_idx = i + 1;
		else
			tmp_gpio_idx = i;

		gpio_value =
			(tmp_gpio_idx >= gpio_ind_to_num_table_cnt) ? 0 : gpio_get(gpio_ind_to_num_table[tmp_gpio_idx]);
...
__weak uint8_t gpio_idx_exchange(ipmi_msg *msg)
{
	...
	if (need_change) {
		...
		else if (msg->data[1] >= WORK_AROUND_BIOS_DEBUG_PIN_IDX_REAL && msg->data[1] < WORK_AROUND_BIOS_DEBUG_PIN_IDX)
			msg->data[1]++;
