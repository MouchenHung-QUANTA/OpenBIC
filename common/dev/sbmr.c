			       ...
			       uint16_t buffer_len)
{
	...
	for (; (i < length) && ((i + start) < current_read_len); i++) {
		memcpy(&buffer[SBMR_POSTCODE_SIZE * i], (uint8_t *)&sbmr_read_buffer[current_index], SBMR_POSTCODE_SIZE);
...
void sbmr_postcode_insert(sbmr_boot_progress_code_t boot_progress_code)
{
	...
	LOG_INF("* [POSTCODE] inst: %02d status:0x%04x code:0x%04x", boot_progress_code.inst, boot_progress_code.status_code, boot_progress_code.efi_status_code);
...
__weak bool SBMR_GET_BOOT_PROGRESS_CODE(ipmi_msg *msg)
{
	...
	if (!sbmr_read_len)
		...
		memcpy(&msg->data[1], (uint8_t *)&sbmr_read_buffer[sbmr_read_len], sizeof(sbmr_boot_progress_code_t));
...
void print_out()
{
	for (int i=0; i<sbmr_read_len; i++) {
		...
		sbmr_read_buffer[i].inst,
		sbmr_read_buffer[i].status_code,
		sbmr_read_buffer[i].efi_status_code);
	}
}
