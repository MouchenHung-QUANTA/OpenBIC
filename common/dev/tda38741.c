...
static bool tda38741_prog_cmd(uint8_t bus, uint8_t addr, uint8_t action, uint8_t offset, uint8_t *rbuf)
{
...
static bool read_tda38741_remaining_wr_reg(uint8_t bus, uint8_t addr, uint8_t reg_offset, size_t read_byte,
	...
	for (i = 0; i < img_size; i++) {
		...
		if (img_buff[i] == 0x2F) {
			...
			if (i + strlen(CNFG_TAG) <= img_size) {
				if (!strncmp(&img_buff[i], CNFG_TAG, strlen(CNFG_TAG))) {
					...
					while (img_buff[i] != 0x0d) {
						...
						dev_cfg->cnfg_data[cnfg_cnt++] = ascii_to_val(img_buff[i])*16 + ascii_to_val(img_buff[i+1]);
			...
			if (i + strlen(CHECKSUM_FIELD) <= img_size) {
				if (!strncmp(&img_buff[i], CHECKSUM_FIELD, strlen(CHECKSUM_FIELD))) {
					i += strlen(CHECKSUM_FIELD);
					i += 2; //pass '0x'
					dev_cfg->checksum = 0;
					dev_cfg->checksum |= (ascii_to_val(img_buff[i]) * 16 + ascii_to_val(img_buff[i+1])) << 24;
					i += 2;
					dev_cfg->checksum |= (ascii_to_val(img_buff[i]) * 16 + ascii_to_val(img_buff[i+1])) << 16;
					i += 2;
					dev_cfg->checksum |= (ascii_to_val(img_buff[i]) * 16 + ascii_to_val(img_buff[i+1])) << 8;
					i += 2;
					dev_cfg->checksum |= (ascii_to_val(img_buff[i]) * 16 + ascii_to_val(img_buff[i+1]));
					i ++;
			...
			if (is_data == true) {
				uint16_t offset = 0;
				offset |= (ascii_to_val(img_buff[i]) * 16 + ascii_to_val(img_buff[i+1])) << 8;
				i += 2;
				offset |= (ascii_to_val(img_buff[i]) * 16 + ascii_to_val(img_buff[i+1]));
				i += 2;
				...
				while (img_buff[i] != 0x0d) {
					...
					dev_cfg->section[sect_cnt].data[data_cnt++] = ascii_to_val(img_buff[i]) * 16 + ascii_to_val(img_buff[i+1]);
					i += 2;
				}
			...
			if (i + strlen(IMAGE_COUNT) <= img_size) {
				if (!strncmp(&img_buff[i], IMAGE_COUNT, strlen(IMAGE_COUNT))) {
					...
					if ( (dev_cfg->sect_count != 1) || (img_buff[i+1] != 0x0d) ) {
	...
	for (int i = 0; i < config->sect_count; i++) {
		...
		for (int j = 0; j < config->section[i].offset_count; j++) {
			...
			for (int k = 0; k < 16; k++) {
				...
				if (i2c_master_write(&i2c_msg, 3)) {
					LOG_ERR("Failed to write program data to VR in section 0x%x offset 0x%x byte 0x%x", i, j, k);
					goto exit;
				}
			}
		}

		if (tda38741_prog_cmd(bus, addr, USER_WR,
								(USER_REMAINING_WRITES_MAX - user_remain), NULL) == false) {
