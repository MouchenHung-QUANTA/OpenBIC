...
bool mp2988_fwupdate(uint8_t bus, uint8_t addr, uint8_t *img_buff, uint32_t img_size)
{
	...
	for (line_idx = 0; line_idx < dev_cfg.wr_cnt; line_idx++) {
		...
		if ((cur_data->page == VR_MPS_PAGE_0) || (cur_data->page == VR_MPS_PAGE_1) || (cur_data->page == VR_MPS_PAGE_2)) {
			...
			LOG_INF("USER page 0x%x wr 0x%x:", cur_data->page, cur_data->reg_addr);
			LOG_HEXDUMP_INF(cur_data->reg_data, cur_data->reg_len, "");
