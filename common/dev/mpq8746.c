...
static bool mpq8746_pre_update(uint8_t bus, uint8_t addr, struct mpq8746_config *dev_cfg)
{
	...
	uint32_t vend_id = (i2c_msg.data[1] << 8) | (i2c_msg.data[2] << 16) | (i2c_msg.data[3] << 24);
	...
	uint8_t dev_id = ((i2c_msg.data[0] | (i2c_msg.data[1] << 8)) >> 13) & BIT_MASK(3); // C1[15:13]

	if (dev_cfg->dev_id != dev_id) {
		LOG_ERR("Invalid device id 0x%x from image, should be 0x%x", dev_cfg->dev_id, dev_id);
		return false;
	}
	...
	if (dev_cfg->cfg_id != cfg_id) {
		LOG_ERR("Invalid config id 0x%x from image, should be 0x%x", dev_cfg->cfg_id, cfg_id);
		return false;
	}
		...
		case ATE_REG_DATA_HEX:
			*((uint32_t *)cur_line->reg_data) = data_store;
			...
			if (cfg_id_get == true) {
				...
				dev_cfg->dev_id = (data_store & GENMASK(15, 13)) >> 13; // 0xC1[15:13]
				dev_id_get = false;
			}
			break;
