...
static bool ssif_do_action(ssif_action_t action, uint8_t smb_cmd, ssif_dev *ssif_inst)
{
	...
	case SSIF_SEND_IPMI:
		if (pal_request_msg_to_BIC_from_HOST(current_ipmi_msg.buffer.netfn, current_ipmi_msg.buffer.cmd)) {
			...
			if (pal_immediate_respond_from_HOST(current_ipmi_msg.buffer.netfn, current_ipmi_msg.buffer.cmd)) {
